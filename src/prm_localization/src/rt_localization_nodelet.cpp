...
//            auto init_yaw = private_nh.param<double>("init_yaw", 0.0);
            ...
            auto icpRANSACOutlierRejectionThreshold = private_nh.param<float>("icpRANSACOutlierRejectionThreshold",
                                                                              0.04f);
...
//        registration.set
            ...
            /** init localization**/
            locali_client = nh.serviceClient<prm_localization::locali>("/localization/relocalization");
            NODELET_INFO("Perform localization ... ");
            prm_localization::locali init_lo;
            if (locali_client.call(init_lo)){
                //init_pose
                curr_pose.setIdentity();
                curr_pose.block(0, 0, 3, 3) = geomo_oritention2rotm(init_lo.response.location.orientation);
                ...
                std::cout<<"curr_pose:\n"<<curr_pose<<std::endl;
                NODELET_INFO("init localization completed! ");
            }else{
                NODELET_WARN("init localization failed! ");
            }
    ...
    private:
        ...
        void lp_odom_callback(const ros::TimerEvent &event) {
            ...
            transformBroadcaster.sendTransform(
                    matrix2transform(event.current_real, predict_ins_pose, map_tf, base_foot_tf));
        ...
        void points_callback(const sensor_msgs::PointCloud2ConstPtr &points_msg) {
            ...
            if (localmap_cloud->empty()) {
                ...
                odom_pub.publish(rotm2odometry(curr_pose, points_msg->header.stamp, map_tf, base_lidar_tf));
                return;
            }
            // convert ros msg
            ...
            //trim and 2d
            auto flat_cloud = trimInputCloud(filtered_cloud, nearPointThreshold, farPointThreshold, trim_low,
                                             trim_high);
            if (flat_cloud->empty()) {
            ...
            if (curr_fitness <= thresholdTimes * regis_threshlod && curr_fitness < regis_hard_threshold) {
                ...
                if (flag_slam_insout) {
                    odometry = rotm2odometry(predict_pose * ins2VeloMli, points_msg->header.stamp, map_tf,
                                             base_foot_tf);
                } else {
...
//                odometry.twist.twist.angular.z = velosity_yaw;
            ...
            //judge bad regis
            if (curr_fitness > thresholdTimes * regis_threshlod || curr_fitness > regis_hard_threshold) {
                NODELET_INFO("cannot match currcloud with lidar %d points, fitness:\t%f", (int) flat_cloud->size(),
                             curr_fitness);
                return;
            }
            fitness_buffer.push_back(curr_fitness);
            regis_threshlod =
                    std::accumulate(fitness_buffer.begin(), fitness_buffer.end(), 0.0f) / fitness_buffer.size();
...
//            NODELET_INFO("time to former stamp = %f seconds",points_msg->header.stamp.toSec()-curr_pose_stamp.toSec());
            ...
            //publish tf
            transformBroadcaster.sendTransform(
                    matrix2transform(points_msg->header.stamp, curr_pose, map_tf, base_lidar_tf));
            transformBroadcaster.sendTransform(
                    matrix2transform(points_msg->header.stamp, transform * ins2VeloMli, map_tf, base_foot_tf));
            ...
            if (flag_slam_insout) {
                ...
                lp_odom_pub.publish(
                        rotm2odometry(transform * ins2VeloMli, points_msg->header.stamp, map_tf, base_foot_tf));
            }
...
//            pcl::transformPointCloud(*curr_cloud,*transformedCloud,transform);
        ...
        pcl::PointCloud<pcl::PointXYZ>::ConstPtr trimInputCloud(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud,
                                                                const float disNear, const float disFar,
...
//            NODELET_INFO("curr avg trim time = %f seconds",time_sum/(++trim_time));
        ...
        void update_velocity(const Eigen::Matrix4f &original_pose, const Eigen::Matrix4f &registered_pose,
                             const ros::Time &oringal_time, const ros::Time &curr_time) {
        ...
        Eigen::Matrix4f
        predict(const Eigen::Matrix4f &original_pose, const ros::Time &oringal_time, const ros::Time &curr_time) const {
        ...
        Eigen::Matrix4f pc_register(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &curr_cloud,
